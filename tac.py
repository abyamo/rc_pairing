...
def main():
    ...
    while turn_counter < 9:
        ...
        turn_input = input(f"Player {current_player}, enter a position 
1-9: ")
...
def isValidInput(turn_input, gameGrid: List) -> bool:
    try:
        if int(turn_input) > 0 and int(turn_input) < 10 and 
gameGrid[int(turn_input)-1] is None:
            return True
        return False
    except:
        return False
...
def winCheck(gameGrid: List) -> bool:  
    # NOTE: For checking - instead of duplicating for X and O, check if 
all three positions in a winning combo contain the same non-null symbol -- 
have to do for loop thru gameGrid to see if satisfies 8 times (# of rows + 
# of cols + 2 for diag) - more optimal way?
    # NOTE: see if more optimal way than for loop thru all 8 winning 
combos > bit masking??
