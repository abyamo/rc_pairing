# Initial scratchpad brainstorming tic-tac-toe game engine requirements & 
possible code org/helper functions
...
for statement that runs 9 times OR till game won:
    ...
    TODO: player alternation logic (hacky back-and-forth? like if p0, then 
p1?) 
    ...
    valid input check:
    - make sure not overwritten: if # non-null, ask player to select 
another box
    ...
    0, 1, 2, 3, 4, 5, 6, 7, 8
      ...
      but how to ignore  state of other indices that may not be relevant? 
some regex format?
...
main func -
    ...
    prompts input for player in while loop till isValidInput - takes valid 
input, 
    updates gameGrid [X or O depending on curr_player; X if curr_player = 
1 and O if player2],
...
# INLINE - whichPlayer(turn_counter) - needed or within main? (turns 0, 2, 
4, 6, 8 are p1 / turns 1, 3, 5, 7, 9 are p2)
#     NOTE: if turn_counter % 2 == 0: 
#         return 1
#     return 2

isValidInput(input, gameGrid[List[int]], curr_player[int])
    NOTE: don't need curr_player parameter here - checking: input is 1-9, 
and NOTE: gameGrid[input-1] is None
    TODO: if not validInput, reprompt logic - here or in main?

printGrid(gameGrid) - needed or within main?
    print(gameGrid)

winCheck(gameGrid[List[int]], curr_player) function - to check if curr 
grid has winning combo
